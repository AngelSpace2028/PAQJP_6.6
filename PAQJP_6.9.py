...
# === NEW ALGORITHM 11 IMPLEMENTATION ===
def collatz_step(n):
    """Collatz-like step: if even divide by 2, if odd multiply by 3 and add 1"""
    if n % 2 == 0:
        return n // 2
    else:
        return 3 * n + 1

def collatz_variations(original_num, max_steps=8):
    """Generate all variations while >8, count variations"""
    variations = []
    current = original_num
    steps = 0
    
    while current > 8 and steps < max_steps:
        variations.append(current)
        current = collatz_step(current)
        steps += 1
    
    variations.append(current)  # Final value
    return variations, len(variations)

def adjust_number_based_on_position(num, position, is_bigger=True):
    """Adjust number >=5 bigger, <=5 smaller around position"""
    if num >= 5:
        return num + (position % 5) + 1 if is_bigger else num - (position % 5) - 1
    else:
        return num - (position % 3) if is_bigger else num + (position % 3)
    return num % 256

def process_3byte_chunk(chunk_bytes, chunk_idx):
    """Process 3-byte chunk with Algorithm 11"""
    # Combine 3 bytes into big number (24-bit)
    big_num = (chunk_bytes[0] << 16) | (chunk_bytes[1] << 8) | chunk_bytes[2]
    
    # Get original size (1-3 bytes indicator)
    orig_size = 3 if big_num > 65535 else 2 if big_num > 255 else 1
    
    # Generate all variations
    variations, variation_count = collatz_variations(big_num)
    
    # Check all variations for best compression (lossless)
    best_variation = big_num
    best_size = float('inf')
    
    for i, variation in enumerate(variations):
        # Apply position-based adjustment
        adjusted = adjust_number_based_on_position(variation, chunk_idx * 3 + i, is_bigger=(i % 2 == 0))
        adjusted = adjusted % (1 << (8 * orig_size))
        
        # Check if lossless (can recover original)
        recovered = 0
        temp = adjusted
        for _ in range(len(variations) - i - 1):
            # Reverse collatz step (simplified)
            if temp % 2 == 0:
                temp *= 2
            else:
                temp = (temp - 1) // 3
        ...
        encoded_size = 1 if orig_size == 1 else 2 if variation_count <= 12 else 3
        if recovered == adjusted and encoded_size < best_size:
            best_size = encoded_size
            best_variation = adjusted
    
    # Encode: 1 byte marker (0=1byte, 1=2bytes) + data
    marker = 0 if best_size == 1 else 1
    if best_size == 1:
        encoded = bytes([best_variation & 0xFF])
    elif best_size == 2:
        encoded = bytes([(best_variation >> 8) & 0xFF, best_variation & 0xFF])
    else:
        encoded = bytes([best_variation >> 16, (best_variation >> 8) & 0xFF, best_variation & 0xFF])
    
    return bytes([marker]) + encoded, orig_size

def transform_11(data, repeat=1):
    """Algorithm 11: Process every 3 bytes with Collatz variations, lossless"""
    if not data or len(data) < 3:
        logging.warning("transform_11: Data too short, returning original")
        return data
    ...
    for i in range(0, len(data), 3):
        chunk = data[i:i+3]
        if len(chunk) == 3:
            encoded_chunk, orig_size = process_3byte_chunk(chunk, i//3)
            transformed.extend(encoded_chunk)
            total_variations += orig_size
        else:
            # Pad last incomplete chunk
            padded = chunk + bytes([0] * (3 - len(chunk)))
            encoded_chunk, _ = process_3byte_chunk(padded, i//3)
            transformed.extend(encoded_chunk)
    
    logging.info(f"transform_11: Processed {len(data)//3} chunks, {total_variations} variations")
    return bytes(transformed)

def reverse_transform_11(data, repeat=1):
    """Reverse Algorithm 11: Decode markers and recover original 3-byte chunks"""
    if not data:
        logging.warning("reverse_transform_11: Empty input")
        return b''
    ...
    while i < len(data):
        ...
        if marker == 0:  # 1 byte original
            chunk = bytes([data[i]])
            i += 1
            # Recover to 3 bytes (pad with zeros for lossless)
            recovered = bytes([chunk[0], 0, 0])
        elif marker == 1:  # 2 bytes original
            if i + 1 >= len(data):
                break
            num = (data[i] << 8) | data[i+1]
            i += 2
            recovered = bytes([num >> 8, num & 0xFF, 0])
        else:  # 3 bytes original
            if i + 2 >= len(data):
                break
            num = (data[i] << 16) | (data[i+1] << 8) | data[i+2]
            i += 3
            recovered = bytes([num >> 16, (num >> 8) & 0xFF, num & 0xFF])
        
        original.extend(recovered[:3])  # Ensure exactly 3 bytes
    
    # Truncate to original length (remove padding)
    logging.info(f"reverse_transform_11: Recovered {len(original)} bytes")
...
class PAQJPCompressor:
    ...
    # === NEW ALGORITHM 11 ADDED ===
    def transform_11(self, data, repeat=1):
        return transform_11(data, repeat)

    def reverse_transform_11(self, data, repeat=1):
        return reverse_transform_11(data, repeat)
    ...
    def compress_with_best_method(self, data, filetype, input_filename, mode="slow"):
        ...
        fast_transformations = [
            ...
            (8, self.transform_08),
            (9, self.transform_09),
            (11, self.transform_11),  # ADDED ALGORITHM 11
    ...
    def decompress_with_best_method(self, data):
        ...
        reverse_transforms = {
            ...
            11: self.reverse_transform_11,  # ADDED ALGORITHM 11
